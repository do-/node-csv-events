...
class CSVEventEmitter extends EventEmitter {
	...
	constructor (options) {
		...
		if (!('mask' in options)) throw Error ('Mask not defined')
		...
		switch (typeof mask) {
			...
			case 'bigint':
				if (mask <= 0) throw Error ('Invalid mask value: ' + mask)
				this.mask = mask
				this.unit = 1n
				break
			...
			default:
				throw Error ('Invalid mask value: ' + mask)
		}
	...
	write (s) {
		...
		for (let i = oldLength; i < length; i ++) {
			...
			switch (c) {
				...
				case CH_COMMA:
					...
					this.to = i
					if ((this.index & this.mask) === this.index) this.emit ('c', this.column)
					this.from = i + 1
					if (c === CH_LF) {
						this.emit ('r')
						this.row ++
		...
		const {length} = this.buf; if (length !== 0) {
			...
			if ((this.index & this.mask) === this.index) {
