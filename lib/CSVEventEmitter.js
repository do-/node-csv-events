const CH_QQ = '"'.charCodeAt (0)
const CH_CR = 13
const CH_LF = 10

const EventEmitter = require ('events')

class CSVEventEmitter extends EventEmitter {

	#mask

	get mask () {

		return this.#mask

	}

	set mask (mask) {

		switch (typeof mask) {

			case 'bigint':
				this.#mask = mask
				this.unit = 1n
				break
				
			case 'number':
				if (!Number.isSafeInteger (mask)) throw Error ('Invalid int mask value: ' + mask)
				this.#mask = mask
				this.unit = 1
				break

			default:
				throw Error (`Invalid mask value: '${mask}' (type: ${typeof mask})`)

		}

	}

	constructor (options) {
	
		super ()

		this.mask = options.mask ?? 0

		this.index = this.unit

		if (!('empty' in options)) options.empty = ''

		this.empty = options.empty	

		if (!('maxLength' in options)) options.maxLength = 1e6
		
		if (!Number.isSafeInteger (options.maxLength)) throw Error ('maxLength must be a safe integer')

		if (options.maxLength <= 0) throw Error ('maxLength must be positive')
		
		this.maxLength = options.maxLength

		if (!('delimiter' in options)) options.delimiter = ','
		
		if (typeof options.delimiter !== 'string') throw Error ('The delimiter must be a string')

		if (options.delimiter.length !== 1) throw Error ('The delimiter must be 1-char string')
				
		this.CH_COMMA = options.delimiter.charCodeAt (0)

		if (!('emptyDoc' in options)) options.emptyDoc = '\r\n'

		if (options.emptyDoc == null) options.emptyDoc = ''
		
		if (typeof options.emptyDoc !== 'string') throw Error ('The emptyDoc must be a string, null or undefined')

		this.emptyDoc = options.emptyDoc
		
		this.buf = ''
		this.from = 0
		this.row = 0n
		this.column = 0
		this.isBalanced = true
		this.isVirgin = true

	}

	get isColumnToReport () {

		if (this.mask === 0) return true

		return (this.index & this.mask) === this.index

	}

	get isToParse () {

		const {buf} = this, {length} = buf
		
		if (length === 0) return false

		if (!this.isVirgin) return true

		const {emptyDoc} = this

		if (length > emptyDoc.length) return true

		return !emptyDoc.startsWith (buf)

	}
	
	write (s, callback) {

		if (s.length === 0) return callback ()
	
		const oldLength = this.buf.length; if (oldLength + s.length > this.maxLength) return callback (Error ('Buffer overflow (maybe a wrongly quoted string)'))

		this.buf += s; if (!this.isToParse) return callback ()
		
		const {buf, CH_COMMA} = this, {length} = buf
		
		for (let i = oldLength; i < length; i ++) {
		
			const c = buf.charCodeAt (i)
			
			switch (c) {
			
				case CH_QQ:
					this.isBalanced = !this.isBalanced
					break

				case CH_LF:
				case CH_COMMA:
					if (!this.isBalanced) break
					this.to = i
					if (this.isColumnToReport) this.emit ('c')
					this.from = i + 1
					if (c === CH_LF) {
						this.emit ('r')
						this.row ++
						this.column = 0
						this.index = this.unit
					}
					else {
						this.column ++
						this.index <<= this.unit
					}
					break

			}
		
		}

		if (this.from !== 0) {
		
			this.buf = buf.slice (this.from)
			
			this.from = 0

			this.isVirgin = false

		}

		callback ()
			
	}

	parseLastCell () {

		if (this.isColumnToReport) {

			this.to = this.buf.length

			this.emit ('c')

		}

		this.emit ('r')

	}
	
	end (s, callback) {

		let err; this.write (s, _ => err = _); if (err) return callback (err)

		if (this.isToParse) this.parseLastCell ()

		callback ()

	}

	get raw () {
	
		let {buf, from, to} = this

		if (buf.charCodeAt (to) === CH_LF && buf.charCodeAt (to - 1) === CH_CR) to --

		return buf.slice (from, to)

	}
		
	get value () {
	
		let {buf, from, to} = this

		if (buf.charCodeAt (to) === CH_LF && buf.charCodeAt (to - 1) === CH_CR) to --

		if (buf.charCodeAt (from) === CH_QQ) {
		
			from ++
			
			to --
		
		}

		if (from === to) return this.empty
		
		const s = buf.slice (from, to)
	
		let pos = s.indexOf ('"'); if (pos === -1) return s

		const last = s.length - 1
		
		let result = '', cur = 0
		
		while (pos !== -1 && cur < last) {
		
			result += s.slice (cur, pos ++)
			
			cur = pos
			
			pos = s.indexOf ('"', cur + 1)
		
		}

		result += cur === last ? '"' : s.slice (cur)

		return result

	}

}

module.exports = CSVEventEmitter