...
class CSVReader extends Transform {
	...
	constructor (options) {
		...
		this.columnClassSelector = options.columnClassSelector ?? (() => (options.columnClass ?? require ('./CSVColumn')))
		...
		const csvOptions = {}; for (const k of ['delimiter', 'empty', 'maxLength', 'mask', 'emptyDoc']) if (k in options) {
		...
		this.addHandler ('c', () => this.onEndOfCell ())
		this.addHandler ('r', () => this.onEndOfLine ())
	...
	addHandler (event, handler) {
		...
		this.csv.on (event, () => {
			...
			try {
				...
				if (!this.emit (event + '-error', err)) this.destroy (err)
	...
	createColumn (i) {
		...
		return new (Reflect.apply (this.columnClassSelector, undefined, arg)) (this, arg)
	...
	onEndOfBodyLine () {
		...
		const value = this.record.valueOf ()
	...
	onEndOfHeaderCell () {
		...
		const {csv} = this, {column} = csv

		if (column >= this.maxColumns) return this.destroy (Error (`Maximum number of columns (${this.maxColumns}) exceeded`))

		const {columnDefinitions} = this

		if (!columnDefinitions [column]) columnDefinitions [column] = []

		columnDefinitions [column] [this.rowNum] = csv.value

	}

	onEndOfBodyCell () {

		this.columns [this.csv.column].copyValue ()
	...
	onEndOfCell () {
		...
		switch (this.state) {
			...
			case ST_HEADER: return this.onEndOfHeaderCell ()

			case ST_BODY:   return this.onEndOfBodyCell ()
	...
	_flush (callback) {
		...
		try {
			...
			this.csv.end (decoder === null ? '' : decoder.end ())
