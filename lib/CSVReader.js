...
class CSVReader extends Transform {
	...
	constructor (options) {
		...
		.on ('c', n => {
			...
			switch (this.state) {
				...
				case ST_HEADER:
					...
					this.columns [n] [this.rowNum - options.skip] = this.csv.value
					break
		...
		if (this.state === ST_BODY) this.onBody ()
	...
	onEndOfLine () {
		...
		const {state} = this; if (state === ST_BODY) return this.onEndOfBodyLine ()

		this [ST2F [state]] --; if (this.state !== ST_BODY) return this.#rowNum ++
